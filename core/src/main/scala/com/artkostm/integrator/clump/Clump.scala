...
sealed trait Clump[+T] {
  ...
  protected[clump] def upstream: List[Clump[_]] = List.empty[Clump[_]]
  protected[clump] def downstream(implicit ec: ExecutionContext): Future[Option[Clump[_]]] = Future.successful(None)
  protected[clump] def result(implicit ec: ExecutionContext): Future[Option[T]]
}
...
private[clump] class ClumpEmpty extends Clump[Nothing] {
  override protected[clump] def result(implicit ec: ExecutionContext) = Future.successful(None)
}

private[clump] class ClumpFuture[T](future: Future[T]) extends Clump[T] {
  override protected[clump] def downstream(implicit ec: ExecutionContext) = future.map(_ => None).recover {
    case _ => None
  }
  override protected[clump] def result(implicit ec: ExecutionContext) = future.map(Some(_))
}

private[clump] class ClumpFetch[T, U](input: T, val source: ClumpSource[T, U]) extends Clump[U] {
  private[this] val promise = Promise[Option[U]]

  protected[clump] def attachTo(fetcher: ClumpFetcher[T, U])(implicit ec: ExecutionContext) =
    fetcher.get(input).onComplete(promise.complete)

  override protected[clump] def result(implicit ec: ExecutionContext) = promise.future
}

private[clump] class ClumpJoin[A, B](a: Clump[A], b: Clump[B]) extends Clump[(A, B)] {
  override protected[clump] val upstream = List(a, b)
  override protected[clump] def result(implicit ec: ExecutionContext) =
...
private[clump] class ClumpCollect[T, C[_] <: Iterable[_]](clumps: C[Clump[T]])(implicit cbf: CanBuildFrom[C[Clump[T]], T, C[T]]) extends Clump[C[T]] {
  override protected[clump] val upstream =
...
private[clump] class ClumpMap[T, U](clump: Clump[T], f: T => U) extends Clump[U] {
  override protected[clump] val upstream = List(clump)
  override protected[clump] def result(implicit ec: ExecutionContext) =
...
private[clump] class ClumpFlatMap[T, U](clump: Clump[T], f: T => Clump[U]) extends Clump[U] {
  ...
  override protected[clump] val upstream = List(clump)
  override protected[clump] def downstream(implicit ec: ExecutionContext) =
...
private[clump] class ClumpHandle[T](clump: Clump[T], f: PartialFunction[Throwable, T]) extends Clump[T] {
  override protected[clump] val upstream = List(clump)
  override protected[clump] def result(implicit ec: ExecutionContext) =
...
private[clump] class ClumpOptional[T](clump: Clump[T]) extends Clump[Option[T]] {
  override protected[clump] val upstream = List(clump)
  override protected[clump] def result(implicit ec: ExecutionContext) = clump.result.map(Some(_))
}
