...
import com.artkostm.integrator.watcher.Messages.EventAtPath

import scala.concurrent.duration.FiniteDuration

private[watcher] case class Occurrence(event: Kind[_], path: Path, storedAt: Long)

object EventTracker {

  private[watcher] def eventToOccurrence(eventAtPath: EventAtPath): Occurrence = {
  ...
  private[watcher] def occurrenceToEvent(occurrence: Occurrence): EventAtPath = {
    EventAtPath(occurrence.event, occurrence.path)
  }

}

private[watcher] case class EventTracker(occurrences: Seq[Occurrence] = Nil) {
