...
object Messages {
  ...
  sealed trait RegisterCallbackMessage {
    ...
    def bossy: Boolean =
      false // "Bossy" means whether or not a message is supposed to remove all other callbacks
  ...
  /**
    ...
    */
  sealed abstract class ForwardToSubscriber(subscriber: ActorRef) {
    this: RegisterCallbackMessage =>
  ...
  /**
    ...
    */
  sealed case class RegisterCallback(event: WatchEvent.Kind[Path],
                                     path: Path,
                                     callback: Callback,
  ...
  sealed case class RegisterSubscriber(event: WatchEvent.Kind[Path],
                                       ...
                                       recursive: Boolean = false,
                                       persistent: Boolean = false)
      extends ForwardToSubscriber(subscriber)
      with RegisterCallbackMessage
  ...
  /**
    ...
    * @param persistent Boolean To automatically add the same callback to new files or not, defaults to false
    */
  sealed case class RegisterBossyCallback(event: WatchEvent.Kind[Path],
                                          path: Path,
                                          callback: Callback,
                                          modifier: Option[Modifier] = None,
                                          recursive: Boolean = false,
  ...
  /**
    ...
    * @param persistent Boolean To automatically add the same callback to new files or not, defaults to false
    */
  sealed case class RegisterBossySubscriber(event: WatchEvent.Kind[Path],
                                            path: Path,
                                            subscriber: ActorRef,
                                            modifier: Option[Modifier] = None,
                                            recursive: Boolean = false,
  ...
  /**
    ...
    * @param path Path (Java object) pointing to a file/directory
    */
  sealed case class UnRegisterCallback(event: WatchEvent.Kind[Path],
                                       path: Path,
                                       recursive: Boolean = false)
